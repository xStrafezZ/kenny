module.exports = async(client, message, args) =>{
  let dinero = Math.floor(Math.random() * 400) + 100
    ...
    // }
let cooldown = new (require("megadb")).crearDB("cooldownk") 
    let tiempo = 7200000
 ...
 if(!cooldown.tiene(`${message.author.id}`)) {
    cooldown.establecer(`${message.author.id}`, Date.now() + tiempo)
     message.channel.send(`Estas minando espera un momento...`).then(m =>{
       setTimeout(() =>{
if(dinero < 120){
  m.edit(`Minaste ${dinero} monedas`)
